...
import math
import time
import threading # 🌟 1. นำเข้า threading
...
class SocketTrackerNode(Node):
    def __init__(self):
        ...
        # 🌟 3. เริ่มรัน Thread โหลดภาพเบื้องหลังทันที
        self.recv_thread = threading.Thread(target=self.receive_frames_background)
        self.recv_thread.daemon = True
        self.recv_thread.start()

    # ==========================================
    # 📌 ส่วนฟังก์ชัน Callbacks
    # ==========================================
    def cb_active(self, msg):
        ...
        if command in ["front", "back", "tracker"]:
            ...
            if self.is_active:
                self.is_active = False
                self.get_logger().info("💤 [TRACKER] Standby")
        
    def cb_robot_ready(self, msg):
        if msg.data: self.waiting_for_robot = False
    ...
    # ==========================================
    # 🌟 4. ฟังก์ชัน Thread โหลดภาพ (ลบปัญหาภาพสะสม)
    # ==========================================
    def receive_frames_background(self):
        data = b""
        ...
        while self.is_receiving:
            ...
            # --- ฮาร์ดแวร์โหมด (Socket) ---
            if not self.camera_ready or self.conn is None:
                try:
                    if self.s is None:
                        os.environ["QT_QPA_PLATFORM"] = "xcb"
                    ...
                    self.conn.settimeout(5.0) # กันเน็ตกระตุก
            ...
            try:
                ...
                # แปลงเป็นภาพ แล้วอัปเดตใส่ latest_frame ทับของเก่าทันที!
    ...
    # ==========================================
    # 📌 ส่วนฟังก์ชันลูปหลัก (Main Loop) เบาลง 10 เท่า!
    # ==========================================
    def run_loop(self):
        try:
            while rclpy.ok(): 
                ...
                if key == ord('q'): break
                elif key == ord('s'): self.cmd_pub.publish(String(data="start"))
                elif key == ord('p'): self.cmd_pub.publish(String(data="preset"))
                elif key == ord('t'):
                    self.cmd_pub.publish(String(data="track"))
                    self.robot_tracking_state = True
                    self.waiting_for_robot = False
                    self.last_r = 0
                elif key == ord('h'): self.cmd_pub.publish(String(data="home"))
                elif key == ord('f'): self.fake_force_pub.publish(String(data="1"))
                ...
                # 🌟 5. ดึงภาพล่าสุดมาประมวลผล (ไม่มีการรอคิวอีกต่อไป)
                if self.latest_frame is None:
                    continue # ถ้ายังไม่มีภาพใหม่ ก็ข้ามไปก่อน ไม่ต้องประมวลผล

                frame = self.latest_frame.copy()
                self.latest_frame = None # เคลียร์ของเก่าทิ้งเพื่อรอรับเฟรมถัดไป
                ...
                if len(boxes) > 0:
                    ...
                    if largest_box is not None:
                        ...
                        if self.last_r == 0 or self.miss_count > 5:
                            self.last_x, self.last_y, self.last_r = raw_x, raw_y, raw_r
                ...
                msg = Point()
                if not found_valid_target:
                    msg.x, msg.y, msg.z = 9999.0, 9999.0, 9999.0
            ...
            self.is_receiving = False # 🌟 สั่งปิด Thread ด้วย
