...
class StepperNode(Node):
    def __init__(self):
        super().__init__('stepper_joint2')

        # ---------- Parameters (ปรับได้จาก launch/CLI) ----------
        self.declare_parameter('pin_ena', 11)     # ENA+ (ENA- → GND)
        ...
        self.declare_parameter('step_delay', 0.0005)      # หน่วงต่อครึ่งคาบ (ควบคุมความเร็ว)
        ...
        self.sub = self.create_subscription(Int32, '/stepper_joint2_cmd', self.cmd_cb, 10)
