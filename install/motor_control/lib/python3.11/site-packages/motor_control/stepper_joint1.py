...
class StepperNode(Node):
    def __init__(self):
        super().__init__('stepper_joint1')

        # ---------- Parameters (ปรับได้จาก launch/CLI) ----------
        self.declare_parameter('pin_ena', 26)     # ENA+ (ENA- → GND)
        self.declare_parameter('pin_dir', 6)      # DIR+ (DIR- → GND)
        self.declare_parameter('pin_pul', 5)      # PUL+ (PUL- → GND)
        self.declare_parameter('ena_active_high', False)   # บางไดรเวอร์ Enable = LOW
        self.declare_parameter('step_delay', 0.0011)      # หน่วงต่อครึ่งคาบ (ควบคุมความเร็ว)
        ...
        self.sub = self.create_subscription(Int32, '/stepper_joint1_cmd', self.cmd_cb, 10)
